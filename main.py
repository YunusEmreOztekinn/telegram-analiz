...
@app.route("/dashboard")
def dashboard():
    return send_from_directory("files", "dashboard.html")
...
async def run_bot():
    ...
    if not TARGET_GROUP:
        print("⚠️  TARGET_GROUP ayarlanmamış! Railway env vars'a ekle.")
        async for dialog in client.iter_dialogs():
            if dialog.is_group or dialog.is_channel:
                u = f"@{dialog.entity.username}" if getattr(dialog.entity, "username", None) else "(username yok)"
                print(f"  {dialog.id:>15}  |  {u:30}  |  {dialog.name}")
        return
    ...
    async def handler(event):
        ...
        target   = TARGET_GROUP.lstrip("@")

        if username != target and chat_id != target:
            return
